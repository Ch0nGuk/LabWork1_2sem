...
void add_polynomials_in_menu(Polynomial *polys[], int *poly_id)
{
    if (*poly_id <= 1)
    {
    ...
    Polynomial* add_pol = create_polynomial(poly1->polynomial_type, poly1->degree >= poly2->degree ? (int)poly1->degree : (int)poly2->degree);

    add_polynomial(poly1, poly2, add_pol);
